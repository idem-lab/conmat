...
-   produces the full names of states in Australia from its abbreviations.

#### age_group_lookup

- A lookup table for the age groups in 5 year bins that a particular age belong to. The dataset contains three variables: age group, its lower and upper limit. 
...
> Are we using this dataset in conmat?
...
> Note : Why group by state inside the switch() ? - I think we can leave this as is. \#### per_capita_household_size
...
> Given some user specified age breaks, get_setting_transmission_matrices returns a named list of matrices containing age-specific relative per-contact transmission probability matrices for each of 4 settings: home, school, work, other while accounting for vaccination, reduced mixing, and reduced transmissibility in work and other settings due to hygiene behaviour. 

It takes three arguments : 

- Age breaks

- An assumption of 0.5 for the infectiousness of asymptomatic relative to symptomatic 

- The Davies et al age specific susceptibility parameter estimates  such as `davies_original` and `davies_updated`.

We use the age dependent clinical fraction from `davies_age_extended` and the assumed relative infectiousness of asymptomatics to symptomatics to obtain the age specific estimate of infectiousness for all age combinations along with the age specific susceptibility estimates.
...
These estimates are then averaged across settings (home, work etc) and age groups such as 0-5,5-10 and so on as per the age breaks specified. 
...
We calculate the estimates of the relative transmissibility in household vs non-household settings and use the `setting_weights` which hold the weights given to each setting regarding transmissibility to scale the probability linearly for non-household transmission and binomially for household transmission so that onward infections do not to exceed the number of other household members.
...
Once infected, a person can transmit an infectious disease to another thereby creating generations of infected individuals. Certain characteristics such as age, sex etc can epidemiologically significant in estimating the expected number of these newly infected individuals. When the population is categorised based on these traits, one can define a matrix that related the number of newly infected individuals in these categories for the consecutive generations. This matrix is called a next generation matrix. Diekmann et al (2010) 

Next generation matrix can be used to calculate the expected number of secondary infections in an age group,say 5-10 caused by an infected individual of same age group or another for example 10-15, assuming that the 5-10 population is entirely susceptible. [check with Nick]
...
By giving a person immunity from the disease, vaccination has consistently demonstrated to be one of the most effective strategies in the management and prevention of an infectious disease outbreak. When a sizable section of the population receives vaccinations or when vaccine coverage is sufficient enough, the likelihood that the unvaccinated population will contract the disease is decreased. This helps to slow infectious disease spread as well as lessen its severity. For this reason, it is important to understand how much of a reduction in probability of acquisition (the likelihood that an individual will contract the disease), and probability of transmission (the likelihood that an individual will spread the disease after contracting it), has occurred as an the effect of vaccination, in other words the effect of vaccination on the next generation of infections.

Conmat applies vaccination effects to next generation matrices using the function `apply_vaccination` that leads to reduction of acquisition and transmission in each age group in the next generation matrices. `apply_vaccination` executes this by taking in the next generation matrices on certain age groups of a specified region obtained through `generate_ngm()` as well as a data set with information on the age group wise vaccine coverage, acquisition and transmission (probabilities?) of the region and age groups corresponding to given next generation matrices.

The function then computes percentage reduction in acquisition and transmission in each age group. By taking the outer product of these reductions in acquisition and transmission by age group, a transmission reduction matrix is created. The next generation matrices with the vaccination effects applied on are then produced using the obtained transmission reduction matrix and the next generation matrices passed to the function as an argument.