...
> Using the fitted model from `fit_single_contact_model()`, `predict_contacts` predict the contact rate between two age groups of a desired population. 

For example, the average number of contacts between age groups 0-5 and 10-15 at the setting, "home" in Fairfield, NSW can be determined using the fitted contact model for the setting, "home", the age wise population of Fairfield, NSW as well as the specified age_breaks such as 5 year breaks from ages 0 to 85. 
...
- The function accepts the model fit, population data on different ages in the desired region as well as the desired age breaks. 

- Using the population data, the maximum and minimum ages for which contact rates are predicted is determined while accounting for the removal of ages with zero population so that predictions are not made for ages with zero populations. 

- The function then uses `predict_contact_1y()` to obtain predictions for all ages between the these minimum and maximum and later on utilises `aggregate_predicted_contacts()` to predict number of contacts in all combinations of the age groups specified by age_breaks.
