...
To fairly compare a dynamic disease model that differs _only_ by it's contact marices, it's important to remember that the $(i,j)$th element of one of these matrices is the *relative* number of contacts between individuals of age $i$ and age $j$. But, what the number is relative to might be different, and this will lead to different basic reproduction numbers, which will give misleading model conclusions.
...
scaling_factor_prem <- optimize(
  f = calculate_R0,
  interval = c(0.001, 5),
  transmission_matrices = transmission_matrix,
  contact_matrices = prem_germany_contact_matrices
)

scaling_factor_socialmixr <- optimize(
  f = calculate_R0,
  interval = c(0.001, 5),
  transmission_matrices = transmission_matrix,
  contact_matrices = germany_contacts
)
