...
names(initial_condition) <- paste(rep(c("S0", "I0", "R0"), each = 9),
...
agestructured_sir <- function(time, state, parameters) {
  ...
  lambda_home <- (
    parameters$transmission_matrix$home * parameters$homogeneous_contact$home
  ) %*%
    state[parameters$i_indexes] / N_by_age

  lambda_work <- (
    parameters$transmission_matrix$work * parameters$homogeneous_contact$work
  ) %*%
    state[parameters$i_indexes] / N_by_age

  lambda_school <- (
    parameters$transmission_matrix$school * parameters$homogeneous_contact$school
  ) %*%
    state[parameters$i_indexes] / N_by_age

  lambda_other <- (
    parameters$transmission_matrix$other * parameters$homogeneous_contact$other
  ) %*%
    state[parameters$i_indexes] / N_by_age
...
homogeneous_soln <- tibble(homogeneous_soln)
...
# Have to convert ode output to a data frame to do any plotting
germany_soln <- tibble(germany_soln)
