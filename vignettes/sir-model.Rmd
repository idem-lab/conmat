...
agestructured_sir <- function(time, state, parameters) {
  ...
  #
  N_by_age <- vapply(parameters[["s_indexes"]], function(i) {
  ...
  lambda_home <- (parameters[["transmission_matrix"]]$home * parameters[["homogeneous_contact"]]$home) %*% state[parameters[["i_indexes"]]] / N_by_age

  lambda_work <- (parameters[["transmission_matrix"]]$work * parameters[["homogeneous_contact"]]$work) %*% state[parameters[["i_indexes"]]] / N_by_age

  lambda_school <- (parameters[["transmission_matrix"]]$school * parameters[["homogeneous_contact"]]$school) %*% state[parameters[["i_indexes"]]] / N_by_age

  lambda_other <- (parameters[["transmission_matrix"]]$other * parameters[["homogeneous_contact"]]$other) %*% state[parameters[["i_indexes"]]] / N_by_age
  ...
  dIdt <- lambda_total * state[parameters[["s_indexes"]]] - parameters[["gamma"]] * state[parameters[["i_indexes"]]]
...
transmission_matrix$home <- matrix(0.1761765, nrow = n_finite_states, ncol = n_finite_states)
transmission_matrix$work <- matrix(0.1761765, nrow = n_finite_states, ncol = n_finite_states)
transmission_matrix$school <- matrix(0.1761765, nrow = n_finite_states, ncol = n_finite_states)
transmission_matrix$other <- matrix(0.1761765, nrow = n_finite_states, ncol = n_finite_states)
...
initial_condition <- c(S0, I0, R0)
names(initial_condition) <- paste(rep(c("S0", "I0", "R0"), each = n_finite_states), age_breaks_socialmixr[1:n_finite_states], sep = "_")
```

```{r solve-ode-germany}
times <- seq(0, 100, by = 0.1)
germany_soln <- ode(y = initial_condition, times = times, func = agestructured_sir, parms = parameters)

# Have to convert ode output to a data frame to do any plotting
germany_soln <- as.data.frame(germany_soln)
...
germany_soln %>%
  ...
  rename(name = parent_state) %>%
  mutate(type = "Age structured") %>%
  ggplot(aes(x = time, y = value / sum(initial_condition), colour = name)) +
  geom_line() +
...
prem_soln <- ode(y = initial_condition, times = times, func = agestructured_sir, parms = parameters_prem)

# Have to convert ode output to a data frame to do any plotting
prem_soln <- as.data.frame(prem_soln)

tail(prem_soln)
```

```{r conmat-prem-plot}
germany_aggregated <- germany_soln %>%
  pivot_longer(!time) %>%
  mutate(parent_state = substr(name, 1, 1)) %>%
  group_by(time, parent_state) %>%
  summarise(value = sum(value)) %>%
...
conmat_prem_soln <- rbind(germany_aggregated, prem_aggregated)
conmat_prem_soln$name <- factor(conmat_prem_soln$name, levels = c("S", "I", "R"))
