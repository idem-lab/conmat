...
For interests sake, let's plot the number of recovered at the final time point

```{r show-recovered}
prem_soln_all_times_recovered <- prem_soln %>%
  select(time, starts_with("R")) %>%
  pivot_longer(
    cols = starts_with("R"),
    names_to = "recovered",
    values_to = "value"
  ...
  mutate(age = as.numeric(
    stringr::str_remove(recovered, "R0_"),
    .after = recovered
  ))

prem_soln_all_times_recovered %>%
  filter(time == 100) %>%
  ggplot(aes(
    x = age,
    y = value
