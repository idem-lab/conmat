...
#' @param household_size bare variable name of the column depicting the household size. Default is 'household_size' from [get_household_size_distribution()].
#' @param n_people bare variable name of the column depicting the total number of people belonging to the respective household size. Default is 'n_people' from [get_household_size_distribution()]. 
#' @return Numeric of length 1 - the per capita household size for a given state
#'     or LGA.
#' @author Nick Golding
#' @export
#' @examples 
#' sample_data <- get_household_size_distribution(lga = "Fairfield (C)")
#' per_capita_household_size(sample_data,household_size,n_people)
#' 
#' 
per_capita_household_size <- function(household_data=sample_data,
