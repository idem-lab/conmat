...
As an example, let us generate a contact matrix for a local area using a model fitted from the POLYMOD data.

Suppose we want to generate a contact matrix for the City of Perth in Australia. We can get the population data for Perth from the helper function `abs_age_lga`:
...
We can plot the resulting contact matrix with `autoplot`:
