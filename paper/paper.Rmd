...
There are currently few options to generate synthetic contact matrices with existing contact surveys. 
Existing methods to generate synthetic contact matrices are not designed for replicability, do not have enough granularity, and does not cover enough administrative areas (some countries are not included).
...
[ What else is covered in this paper? ] The implemented methods are tested and an example of its use is provided for a local government area (i.e. sub-national level) in Australia.
...
The six terms are $|i-j|$, ${|i-j|}^{2}$, $i + j$, $i \times j$, $\text{max}(i, j)$ and $\text{min}(i, j)$.
...
[ #TODO Change the following sentence, copied ad verbatim from JSS bizicount because I like it ] For now, however, we feel that our base `bizicount` package is sufficiently general to assist in estimating the models most often encountered by applied researchers.
